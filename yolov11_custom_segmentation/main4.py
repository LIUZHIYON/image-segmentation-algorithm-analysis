...
class MWindow(QtWidgets.QMainWindow):
    ...
    def openImageFile(self):  # 新增读取图片的功能
        ...
        if imagePath:
            ...
            # 获取处理后的图像，并确保它是彩色图
            img_treated = results.plot(line_width=2)  # 默认返回的是彩色图像
    ...
    def displayResults(self, results):
        ...
        # 打印 results 的结构，查看里面包含哪些数据
        print("Results:", results)  # 调试输出，检查返回结果的结构

        # 如果 results 是一个列表，可能需要访问其中的每个结果
        for result in results:
            print(result)  # 输出每个检测结果
            boxes = result.boxes  # 获取目标检测框
            names = result.names  # 获取类别名称
            if len(boxes) > 0:
                for i in range(len(boxes)):  # 逐个处理目标
                    label = names[int(boxes[i, 5])]  # 获取目标的类别
                    confidence = boxes[i, 4]  # 获取目标的置信度
                    text = f"标签: {label}, 置信度: {confidence:.2f}\n"
