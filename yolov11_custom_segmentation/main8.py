...
class MWindow(QtWidgets.QMainWindow):
    ...
    def load_mask_rcnn_model(self):
        """加载本地训练好的 Mask R-CNN 模型"""
        cfg = get_cfg()

        # 手动设置模型配置
        cfg.merge_from_file("configs/COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml")  # 使用默认配置文件
        cfg.MODEL.WEIGHTS = "detectron_ckps/model_final.pth"  # 本地训练好的模型权重文件
        cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5  # 置信度阈值
        cfg.MODEL.DEVICE = "cuda"  # 使用 CPU 或 "cuda" 使用 GPU

        # 设置类别数量（根据你的数据集）
        cfg.MODEL.ROI_HEADS.NUM_CLASSES = 4  # 替换为你的数据集类别数量

        # 加载元数据（类别名称）
        MetadataCatalog.get("custom_dataset").thing_classes = ["person", "roadheader", "robot", "shearer"]  # 替换为你的类别名称
    ...
    def openImageFile(self):
        ...
        if imagePath:
            ...
            if self.current_model == "YOLO":
                ...
                v = Visualizer(img[:, :, ::-1], metadata=MetadataCatalog.get("custom_dataset"),
    ...
    def frameAnalyzeThreadFunc(self):
        while True:
            ...
            if self.current_model == "YOLO":
                ...
                v = Visualizer(frame[:, :, ::-1], metadata=MetadataCatalog.get("custom_dataset"),
                               scale=0.8, instance_mode=ColorMode.SEGMENTATION)
                v = v.draw_instance_predictions(outputs["instances"].to("cpu"))
                img = v.get_image()[:, :, ::-1]

            # 将输出图片缩放到与输入图片相同的尺寸
            img = cv2.resize(img, (int(frame.shape[1] * self.image_scale), int(frame.shape[0] * self.image_scale)))
