...
class VideoProcessingThread(QtCore.QThread):
    ...
    frame_processed = QtCore.Signal(np.ndarray, object)  # 发射处理后的帧和结果
    ...
    def __init__(self, video_path, output_path, model_type, yolo_model=None, mask_rcnn_model=None, parent=None):
        ...
        self.original_size = None  # 存储原始视频尺寸
    ...
    def process_frame(self, frame):
        ...
        return cv2.cvtColor(segmented_frame, cv2.COLOR_RGB2BGR), results
    ...
    def run(self):
        try:
            ...
            self.original_size = (width, height)  # 保存原始尺寸
            ...
            while self._is_running and cap.isOpened():
                ...
                # 写入视频文件(保持原始尺寸)
                out.write(cv2.resize(processed_frame, (width, height)))

                # 为显示调整尺寸(不超过UI限制)
                display_frame = self.resize_for_display(processed_frame)

                # 发射信号用于实时显示和结果
                self.frame_processed.emit(display_frame, results)
...
class MWindow(QtWidgets.QMainWindow):
    ...
    def saveVideoResult(self):
        ...
        self.video_processing_thread.frame_processed.connect(self.update_processed_frame)
    ...
    def update_processed_frame(self, frame, results):
        """更新处理后的帧显示"""
        self.current_result = results
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        self.displayImage(frame, self.label_treated)
        ...
        if self.current_model == "YOLO":
            ...
            for i, score in enumerate(results["instances"].scores):
                class_id = results["instances"].pred_classes[i].item()
    ...
    def frameAnalyzeThreadFunc(self):
        """帧分析线程函数"""
        while True:
            if not self.frameToAnalyze or self.is_processing_video:
                time.sleep(0.01)
                continue
